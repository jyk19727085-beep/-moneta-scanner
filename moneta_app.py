...
st.markdown("객관적 수급 데이터와 기술적 분석을 결합한 퀀트 투자 인사이트 시스템입니다.")
...
with tab1:
    ...
    st.markdown("시장의 전반적인 유동성과 위험 선호도를 파악하기 위한 객관적 지표입니다.")
...
with tab2:
    ...
    st.markdown("당일 외국인/기관 쌍끌이 매수 상위 종목을 추출하고, 기술적 위치(RSI, 볼린저 밴드)를 객관적으로 분석합니다.")
    
    # 버튼을 눌러야만 스캐닝 시작 (불필요한 과부하 방지)
    if st.button("🚀 종목 스캐닝 시작", type="primary"):
        with st.spinner("KRX 수급 데이터 및 기술적 지표를 분석 중입니다. 약 10~20초 소요됩니다..."):
            ...
            if top_df.empty:
                ...
                if results:
                    ...
                    # 데이터프레임 스타일링
                    st.dataframe(
                        final_df.style.map(lambda x: 'color: red' if isinstance(x, str) and '🔴' in x else ('color: blue' if isinstance(x, str) and '🔵' in x else ''), subset=['기술적분석'])
                                      .format({"전일비(%)": "{:.2f}%", "현재가": "{:,.0f}"}),
                        use_container_width=True,
                        height=500
                    )
                    
                    st.caption("※ 본 데이터는 장 마감 후 업데이트된 수치 기준이며, 기술적 분석 의견은 투자를 위한 절대적 지표가 아닌 객관적 참고용입니다.")
                else:
